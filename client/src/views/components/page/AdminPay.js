...
import { Tab, Row, Col, Nav, Table, Button, Spinner } from "react-bootstrap";
...
export default function AdminPay() {
  const {
    orderState: { orders, orderLoading, ordersLoading },
    getOrder,
    setShowView,
    setShowUpdate,
    getOrderDetail,
    showDelete: { show, idOrder },
    setShowDelete,
    deleteOrder,
    updateOrder,
  ...
  const orderSpending = orders.filter((order) => {
    return order.check === false;
  });
  const orderConfirm = orders.filter((order) => {
    return order.check === true;
  });
  if (ordersLoading) {
    return (
      <div
                    ...
                    {orderConfirm.map((order, index) => (
                      <tr key={order._id}>
                        ...
                        <td className="d-flex justify-content-center">
                          ...
                          <Button
                            onClick={() => choseOrderView(order._id)}
                          ...
                          <Button
                            onClick={() => choseOrderUpdate(order._id)}
